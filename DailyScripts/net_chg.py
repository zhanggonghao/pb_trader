...
class MarketDataFetcher:
    ...
    def get_futures_info(self) -> pd.DataFrame:
        ...
        try:
            ...
            price_df = price_df.rename(columns={'order_book_id': 'code', 'prev_close': 'preclose', 'prev_settlement': 'presettlement'})
    ...
    def get_stock_price_info(self, time_periods: List[Tuple[str, str]]) -> pd.DataFrame:
        ...
        try:
            ...
            predaily = rqdatac.get_price(
                ...
                fields=['close'],adjust_type='pre', skip_suspended=False, market='cn', expect_df=True).reset_index().rename(columns={'close': 'prev_close'})
            ...
            daily['allday_vwap'] = np.where(daily['volume'] > 0, daily['total_turnover'] / daily['volume'], daily['close'])
            ...
            for st, et in time_periods:
                ...
                grouped = period_data.groupby(['order_book_id', 'date'])[['total_turnover', 'volume']].sum().reset_index()
                grouped[label] = np.where(grouped['volume'] > 0, grouped['total_turnover'] / grouped['volume'], 0)
                daily = pd.merge(daily, grouped[['order_book_id', 'date', label]], on=['order_book_id', 'date'], how='left')
        except Exception as e:
...
class ProductNetCalculator:
    ...
    def _download_emails(self):
        ...
        for name in product_chinese_names:
            ...
            if not match_file or not os.path.exists(match_file):
                try:
                    manager.download_attachments_by_keyword([name, format_predate], save_dir=self.net_email_path, file_extensions=['.xlsx'])
                except Exception as e:
    ...
    def _read_product_files(self, product: str, subdir: str, suffix: str = '', date: str = None) -> Optional[pd.DataFrame]:
        """统一读取产品相关文件（支持多文件合并）"""
        if date is None:
            date = self.date
    ...
    def _get_previous_net(self, product: str) -> Tuple[float, float, float]:
        ...
        pre_file = os.path.join(self.standard_path, 'net_data', self.pre_date, f'{self.pre_date}_{product}_net_info.csv')
    ...
    def _calc_futures_diff_pnl(self, product: str) -> Tuple[float, float, float]:
        ...
        fut_pos['diff_pnl'] = (fut_pos['settlement'] - fut_pos['close']) * fut_pos['Direction'] * fut_pos['vol'] * fut_pos['contract_multiplier']
        ...
        if not cta.empty:
            ...
            margin_tmp['settlement_amount'] = np.where(margin_tmp['settlement_amount'] == 0, margin_tmp['abs_amount'] * 0.5, margin_tmp['settlement_amount'])
            # print(margin_tmp)
    ...
    def _calc_futures_order_pnl(self, product: str) -> Tuple[float, float, float, float]:
        ...
        fut_order['order_pnl'] = fut_order['Direction'] * fut_order['filled_vol'] * fut_order['contract_multiplier'] * (fut_order['presettlement'] - fut_order['filled_price'])
    ...
    def _calc_futures_pos_pnl(self, product: str) -> Tuple[float, float, float, float, float, float]:
        ...
        quant_hold_pnl = ((quant['settlement'] - quant['presettlement']) * quant['vol'] * quant['contract_multiplier'] * quant['Direction']).sum() if not quant.empty else 0.0
        quant_diff_pnl = ((quant['settlement'] - quant['close']) * quant['Direction'] * quant['vol'] * quant['contract_multiplier']).sum() if not quant.empty else 0.0
        ...
        cta_hold_pnl = ((cta['settlement'] - cta['presettlement']) * cta['vol'] * cta['contract_multiplier'] * cta['Direction']).sum() if not cta.empty else 0.0
        cta_diff_pnl = ((cta['settlement'] - cta['close']) * cta['Direction'] * cta['vol'] * cta['contract_multiplier']).sum() if not cta.empty else 0.0
    ...
    def _deal_stock_order(self, acct: str, adj_fee_ratio=0.00015, t0_fee_ratio=0.0002):
        ...
        if not adj_data.empty:
            ...
            stk_lng_adj_slipvwap = (adj_data['benchmark_slipvwap_amount'].sum() / stk_lng_adj_amount) if stk_lng_adj_amount != 0 else 0
    ...
    def calculate_product(self, product: str, info: Dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        quant_hold_val, quant_hold_pnl, quant_diff_pnl, cta_hold_val, cta_hold_pnl, cta_diff_pnl = self._calc_futures_pos_pnl(product)
        ...
        product_df = pd.DataFrame([{
            ...
            '对冲比例': abs(stk_hold / quant_fut_hold_value) if quant_fut_hold_value != 0 else (np.nan if product_type == 'zx' else 0),
            '期权总资产': opt_assets,
            '基准': benchmark_chg,
    ...
    def calculate_account(self, product: str, chinese_name: str, benchmark: str, acct: str, acct_info: Dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """计算单个账户的股票交易和持仓明细"""
...
class ReportGenerator:
    ...
    def display_product_summary(df: pd.DataFrame):
        ...
        display_df['产品净资产(结算价)'] = display_df['产品净资产(结算价)'].apply(lambda x: ReportGenerator.format_money(x))
    ...
    def display_futures_summary(df: pd.DataFrame):
        ...
        if df.empty:
            return
        display_df = df.copy()
        for col in ['昨日期货静态权益', '期货户静态权益', '期货户出入金', '期货户收益', '期货户保证金', 'quant收益', 'cta收益', 'quant+cta收益']:
    ...
    def display_account_summary(df: pd.DataFrame):
        ...
        if df.empty:
            return
            # 多头T0收益
        display_df = df[['日期', '产品名', '账户名', '证券户净资产', '多头持仓市值', '证券户总收益', '基准收益率', '多头超额', '多头超额(含T0)']].copy()
        tmp = df[['日期', '产品名', '账户名', '证券户净资产', '多头持仓市值', '证券户总收益', '多头T0收益', '基准收益率', '多头超额', '多头超额(含T0)']].copy()
        tmp = tmp[tmp['产品名'] == '配邦恒升中性1号'].copy()
        if isinstance(tmp, pd.DataFrame) and (not tmp.empty):
            tmp = tmp.groupby(['日期', '产品名']).agg({'证券户净资产': 'sum', '多头持仓市值': 'sum', '证券户总收益': 'sum', '多头T0收益': 'sum', '基准收益率': 'mean', '多头超额': 'mean', '多头超额(含T0)': 'mean'}).reset_index()
            tmp['账户名'] = '配邦恒升合计'
            tmp['多头超额'] = tmp['证券户总收益'] / tmp['多头持仓市值'] - tmp['基准收益率']
            tmp['多头超额(含T0)'] = (tmp['证券户总收益'] + tmp['多头T0收益']) / tmp['多头持仓市值'] - tmp['基准收益率']
            display_df = pd.concat([display_df, tmp[['日期', '产品名', '账户名', '证券户净资产', '多头持仓市值', '证券户总收益', '基准收益率', '多头超额', '多头超额(含T0)']]])
        display_df['证券户净资产'] = display_df['证券户净资产'].apply(lambda x: ReportGenerator.format_money(x))
    ...
    def display_t0_summary(df: pd.DataFrame):
        ...
        if df.empty:
            return
        display_df = df[['日期', '产品名', '账户名', 't0_algo', '多头T0换手率(双边)', '多头T0收益', '多头T0收益率', 'adj_algo', '多头调仓滑点']].copy()
...
def main():
    ...
    if all_product_nav:
        ...
        # 保存汇总文件
        combined_nav.to_csv(os.path.join(calculator.standard_path, 'net_data', date, f"{date}_all_product_info.csv"), index=False)
