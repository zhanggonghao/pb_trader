...
class MyRQFactorsBase(object):
    ...
    def get_factor(self, factor_name, funcs=(), ts_kwargs=None, **kwargs):
        ...
        for func in funcs:
            ...
            if '_' + method in self.get_tsf_adjust_methods():
                ...
                logger.warning(
                    "对因子<{}>执行TSFresh<{}>运算，窗口为: {}，参数为{}".format(factor_name, method, window, ts_kwargs))
            else:
    ...
    def _get_circulation_a_shares(order_book_ids, start_date, end_date):
        ...
        df = rqdatac.get_shares(order_book_ids, start_date, end_date, 'circulation_a', expect_df=False).reindex(
            columns=order_book_ids)
        return df
...
class Fundamentals(MyRQFactorsBase):
    ...
    def rq_ctop(self):
        ...
        """
        return self._to_rq_expression(
            '(cash_flow_from_operating_activities_ttm_0 + cash_flow_from_investing_activities_ttm_0 + cash_flow_from_financing_activities_ttm_0)/market_cap_3')
...
class MyRQFactors(MyRQVolumeFactors, MyRQPriceFactors, MACD, BBand):
    ...
    def _rq_dmi(self, window=12):
        ...
        """
        # ta.dm()
        tr_ = SUM(MAX(MAX(Factor('high') - Factor('low'), ABS(Factor('high') - REF(Factor('close'), 1))),
                      ABS(Factor('low') - REF(Factor('close'), 1))), window)
    ...
    def _rq_adtm(self, window=23):
        ...
        MAADTM = MA(ADTM, M)
        """
        dtm_ = IF(Factor('open') <= REF(Factor('open'), 1), 0,
                  MAX((Factor('high') - Factor('open')), (Factor('open') - REF(Factor('open'), 1))))
        dbm_ = IF(Factor('open') >= REF(Factor('open'), 1), 0,
                  MAX((Factor('open') - Factor('low')), (Factor('open') - REF(Factor('open'), 1))))
    ...
    def _get_factor_ad(self, order_book_ids, start_date, end_date):
        close = rqdatac.get_price(order_book_ids, start_date, end_date, fields='close', expect_df=False,
                                  adjust_type='post')
        high = rqdatac.get_price(order_book_ids, start_date, end_date, fields='high', expect_df=False,
                                 adjust_type='post')
        low = rqdatac.get_price(order_book_ids, start_date, end_date, fields='low', expect_df=False, adjust_type='post')
        volume = rqdatac.get_price(order_book_ids, start_date, end_date, fields='volume', expect_df=False,
                                   adjust_type='post')
...
def get_factor(factor_name, order_book_ids, start_date, end_date, factor_type=None, funcs=(), ts_kwargs={}, **kwargs):
    ...
    if factor_type in ['base', 'deri']:
        ...
        factor_ = my_rq_factors.get_factor(factor_name, funcs=funcs, ts_kwargs=ts_kwargs,
                                           **kwargs)  # my_rq_factors[factor_name]()
    ...
    **kwargs : get_factor中传入的参数，主要是因子的参数，例如window等
    ...
    if df_factor.empty:
        ...
        df_factor = get_factor(factor_name, assets, s_date, e_date,
                               factor_type=factor_type, funcs=ts_funcs, ts_kwargs=ts_kwargs, **kwargs).stack().rename(
            new_factor_name)
        df_factor.index.names = [date_field, asset_field]
        df_all = base_dataframe.merge(df_factor.reset_index(), on=[date_field, asset_field], how='left')
        # 针对列表输入方式继续搜索截面算法
        ...
        if len(new_cs_funcs) == 1:
            ...
            if len(new_cs_kwargs) > 0:
                df_all.rename(columns={new_factor_name: '__'.join(
                    [new_factor_name, cs_func, "__".join([f"{k}_{v}" for k, v in new_cs_kwargs.items()])])},
                              inplace=True)
                new_factor_name = '__'.join(
                    [new_factor_name, cs_func, "__".join([f"{k}_{v}" for k, v in new_cs_kwargs.items()])])
            else:
                df_all.rename(columns={new_factor_name: new_factor_name + '__' + cs_func}, inplace=True)
                new_factor_name += '__' + cs_func
...
def update_rqfactors_daily(factor_data_1d_path: str):
    ...
    for factor_name in factors:
        ...
        if os.path.exists(fp_factor):
            ...
            logger.warning(
                f"{factor_name}因子尚未生成过，{fp_factor}将从默认起点:{update_start_date}开始生成数据，耗时较久")
        ...
        end_time = time.time()
        logger.info(
            f"{factor_name}因子更新完成 {fp_factor}写入完成, 耗时：{round(end_time - start_time, 2)}s {round(len(df_all_factor_data) / (end_time - start_time), 2)}row/s")
...
### Customized
class ZYFactors(MyRQFactorsBase):
    @staticmethod
    def rq_vwap():
        return Factor('total_turnover') / Factor('volume')

    @staticmethod
    def rq_price3():
        return (Factor('high')+Factor('low')+Factor('close'))/3

### Multi-Inherance
class MyFactors(WorldQuantAlpha101, Fundamentals, NPVI, RSRS, RSI, WilliamRate, ATR, Turnover, ZYFactors):
    """因子集合与统一输出层
