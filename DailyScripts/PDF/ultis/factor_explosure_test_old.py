...
def load_index_weights_panel(
    ...
    cap_method:
      - 'adjusted' (默认): 优先取本地数据库存储的官方权重 (与基准侧口径一致),
        加载失败时回退到分级靠档调整市值在指数自身的归一化。
      - 'lcap'    : 旧版 lcap/exp(lcap) 因子口径, 用于回退对比。
...
def _compute_self_weight_panel(
    ...
    """
    计算 index_code 自身的成分股权重, 输出长表 [date, order_book_id, weight]。

    cap_method:
      - 'adjusted' (默认): 优先使用本地数据库存储的官方权重
        (StockDataClient.get_stock_index_comments_weights_industry);
        若加载失败则回退到分级靠档调整市值占比口径
        (adj_shares × 除权报价 在指数自身的归一化)。
      - 'lcap': 旧版口径, weight_i = exp(lcap_i) / Σ exp(lcap_k)。
    """
    if cap_method == 'adjusted':
        # 1) 直接走本地基准权重 (index 自身, 等价于 in-benchmark 全覆盖)
        try:
    ...
    factors['weight'] = factors['mkt_cap_proxy'].astype(float) / factors['_total'].astype(float)

    out = factors[['date', 'order_book_id', 'weight']].copy()
    ...
    if n_days > 0:
        daily_sum = out.groupby('date')['weight'].sum()
        _log(
            f'本地市值加权成分股权重计算完成: index={index_code}, '
            f'cap_method={cap_method}(fallback), rows={len(out)}, days={n_days}, '
...
def main(SAMPLE_RANGE, BENCHMARK, START, DATA_PATH, WINDOW):
    ...
    # 1) 样本指数（默认 000906.XSHG）成分股权重，用于构造横截面候选集合
    _log(f'开始加载样本指数权重: sample={SAMPLE_RANGE}')
    sample_weights = load_index_weights_panel(client=client, order_book_id=SAMPLE_RANGE, start=START)
    # sample_weights = 
    ...
    benchmark_weights = load_index_weights_panel(client=client, order_book_id=BENCHMARK, start=START)
