...
class StandardTradeData:
    def __init__(self, date):
        ...
        self.transfer = LinuxFileTransfer(hostname="192.168.3.100", username="crypto", password="crypto", port=22)
    ...
    def _load_config(self):
        ...
        if not os.path.exists(local_config):
            shutil.copy(os.path.join(ROOT_PATH, 'generate_split_system', 'split_system.yaml'), local_config)
    ...
    def get_qmt_data(self):
        """获取 QMT 导出的原始数据并保存"""
        qmt_path = "E:/qmt_auto_export" # 写死目录
    ...
    def standard_fut_data(self):
        """标准化期货数据（无限易和手动）"""
        fut_auto_dir = "E:/future_auto" # 写死目录
    ...
    def standard_entrust(self):
        ...
        for product, pinfo in self.product_info.items():
            for acct in pinfo.get('acct_info', {}):
                ...
                remote_paths = f"{self.standard_path}/entrust/{self.date}/{self.date}_{acct}_entrust.csv"
                local_paths = f"{self.local_standard_path}/entrust/{self.date}/{self.date}_{acct}_entrust.csv"
                data.to_csv(remote_paths, index=False)
                data.to_csv(local_paths, index=False)
    ...
    def standard_pos(self):
        ...
        for product, pinfo in self.product_info.items():
            for acct in pinfo.get('acct_info', {}):
                ...
                data.to_csv(local_file, index=False)
                remote_paths = f"{self.standard_path}/pos/{self.date}/{self.date}_{acct}_pos.csv"
                local_paths = f"{self.local_standard_path}/pos/{self.date}/{self.date}_{acct}_pos.csv"
                data.to_csv(remote_paths, index=False)
                data.to_csv(local_paths, index=False)
                # self.transfer.upload_file(local_file, remote_path)
    ...
    def standard_assets(self):
        ...
        for product, pinfo in self.product_info.items():
            for acct in pinfo.get('acct_info', {}):
                ...
                data.to_csv(local_file, index=False)
                remote_paths = f"{self.standard_path}/assets/{self.date}/{self.date}_{acct}_assets.csv"
                local_paths = f"{self.local_standard_path}/assets/{self.date}/{self.date}_{acct}_assets.csv"
                data.to_csv(remote_paths, index=False)
                data.to_csv(local_paths, index=False)
    ...
    def upload_derived_data(self):
        ...
        remote_fut_dir = f"{self.standard_path}/fut/{self.date}"
        local_fut_dir = f"{self.local_standard_path}/fut/{self.date}"
        if os.path.exists(fut_dir):
            for file in os.listdir(fut_dir):
                ...
                shutil.copy(full_path, f"{remote_fut_dir}/{file}")
                shutil.copy(full_path, f"{local_fut_dir}/{file}")

        opt_dir = os.path.join(ROOT_PATH, "data/standard_trade_data/opt_data", self.date)
        remote_opt_dir = f"{self.standard_path}/opt/{self.date}"
        local_opt_dir = f"{self.local_standard_path}/opt/{self.date}"
        if os.path.exists(opt_dir):
            for file in os.listdir(opt_dir):
                ...
                shutil.copy(full_path, f"{remote_opt_dir}/{file}")
                shutil.copy(full_path, f"{local_opt_dir}/{file}")
    ...
    def main(self):
        ...
        local_base = self.local_standard_path
        dirs = ['pos', 'entrust', 'assets', 'fut', 'opt']
        for d in dirs:
            os.makedirs(f"{remote_base}/{d}/{self.date}", exist_ok=True)
            os.makedirs(f"{local_base}/{d}/{self.date}", exist_ok=True)
